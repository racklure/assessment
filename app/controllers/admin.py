...
from app.models.assessment import (
    ...
    Assessee, AssessmentRecord, AssessmentTemplate
)
...
def assessments():
    """考核管理列表"""
    assessments_list = Assessment.query.order_by(Assessment.created_at.desc()).all()
...
def view_assessment(assessment_id):
    ...
    # 获取模板项目
    template_items = assessment.template.items if assessment.template else []
    ...
    stats = {
        ...
        'average_score': (sum(r['total_score'] for r in completed_records) / len(completed_records)) 
                        if completed_records else 0
    }
    
    # 检查是否可以评估
    can_assess = (
        current_assessor is not None  # 是考核人
        and assessment.is_active  # 考核处于活动状态
        and (current_record is None or not current_record.is_submitted)  # 未提交评估或尚未评估
    )
...
def start_assessment(assessment_id):
    ...
    # 创建考核记录
    @admin_bp.route('/assessment-records/<int:record_id>')
    @admin_required
    @handle_db_error
    def view_record(id):
        """查看考核记录详情"""
        ...
        for item in template.items:
            item_data = item.copy()  # 复制一份，避免修改原始数据
            item_data['score'] = scores.get(item['name'], 0)  # 从评分记录中获取分数
